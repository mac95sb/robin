...
- Prefer native HTML and CSS for interaction. Use `Tabs`, disclosures, forms, and Invoker Commands before browser state; Robin generates required identifiers, so application code must not register per-widget scripts or wire controls with custom IDs. Never add a `*ClientModule`: use primitive state, Invoker Commands, or a framework-owned typed manifest record for an unavoidable browser capability.
