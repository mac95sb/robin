...
/// Persists message history and publishes realtime updates.
actor MessageStore {
  ...
  func all(at now: Date = Date()) async throws -> [ChatMessage] {
    ...
    return Array(try JSONDecoder().decode([ChatMessage].self, from: data).suffix(Self.maximumMessages))
  }

  func append(_ proposed: String, authorID: String, at now: Date = Date()) async throws
    -> ChatMessage
  {
    ...
    while true {
      ...
      var messages = try previous.map { try JSONDecoder().decode([ChatMessage].self, from: $0) } ?? []
