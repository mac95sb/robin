...
struct HealthController: Controller {
  ...
  /// successful request returns HTTP 200 with `{"status":"ok"}`. Robin's normal malformed-request
  /// and internal-error responses still apply.
  ///
  /// Readiness checks may use the deployment's standard infrastructure rate limit. The operation is
  /// safe to retry and needs no idempotency key.
  ///
