...
extension ClientNavigation {
  ...
  public var runtimeModule: String? {
    ...
    return #"""
      const load=async(url,push)=>{try{const response=await fetch(url,{headers:{Accept:"text/html"}});if(!response.ok)throw 0;const next=new DOMParser().parseFromString(await response.text(),"text/html");const swap=()=>{document.title=next.title;document.documentElement.lang=next.documentElement.lang;document.body.replaceWith(next.body);if(push)history.pushState({},"",url)};document.startViewTransition?document.startViewTransition(swap):swap()}catch{location.assign(url)}};
