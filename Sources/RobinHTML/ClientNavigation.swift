...
extension ClientNavigation {
  ...
  public var runtimeModule: String? {
    ...
    return #"""
      const load=async(url,push)=>{try{const response=await fetch(url,{headers:{Accept:"text/html"}});if(!response.ok)throw 0;const next=new DOMParser().parseFromString(await response.text(),"text/html");if(next.querySelector("[data-robin-text],[data-robin-input],[data-robin-action],[data-robin-hidden],[data-robin-disabled],[data-robin-visible],[data-robin-change],[data-robin-edit]")&&!document[Symbol.for("robin.state")]){location.assign(url);return}const wanted=[...next.querySelectorAll('link[data-robin-style]')],current=[...document.querySelectorAll('link[data-robin-style]')],have=new Set(current.map(link=>link.href));await Promise.all(wanted.filter(link=>!have.has(link.href)).map(link=>new Promise((resolve,reject)=>{const copy=link.cloneNode();copy.onload=resolve;copy.onerror=reject;document.head.append(copy)})));const swap=()=>{document.title=next.title;document.documentElement.lang=next.documentElement.lang;document.body.replaceWith(next.body);const keep=new Set(wanted.map(link=>link.href));current.filter(link=>!keep.has(link.href)).forEach(link=>link.remove());if(push)history.pushState({},"",url)};document.startViewTransition?document.startViewTransition(swap):swap()}catch{location.assign(url)}};
