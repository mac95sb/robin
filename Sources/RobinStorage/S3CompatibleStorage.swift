...
public final class S3CompatibleStorage: Storage, StorageIntentSigner, Sendable {
  ...
  private func s3Key(_ key: ScopedObjectKey) -> String {
    "\(configuration.keyPrefix)/\(key.storageIdentifier)"
  }
...
// One synchronous parser owns this delegate; it never crosses a task boundary.
private final class S3Listing: NSObject, XMLParserDelegate {
...
private final class S3TemporaryFile: Sendable {
  let url: URL
